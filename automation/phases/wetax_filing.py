...
async def _file_one(page, file_path: str, inp: Inputs, log) -> tuple[bool, str]:
    ...
    ok, rows = await W.convert_and_verify(page, log)
    if not ok:
    ...
    if ok and rows:
        # 방금 신고한 업체 명단을 작업 대장에 기록 → 납부서 출력이 이 명단을 순회
        n = roster.record_wt_rows(rows, datetime.now().strftime("%Y-%m-%d"))
        log(f"[i] (위택스) 작업 대장에 {n}건 기록")
